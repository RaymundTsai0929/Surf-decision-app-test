...
st.markdown(
    ...
    <style>
      ...
      #boot-loader-mask.hide {
        opacity: 0;
        visibility: hidden;
      }
      #boot-loader-mask .spinner {
        width: 34px;
        height: 34px;
        border-radius: 50%;
    ...
    <div id="boot-loader-mask">
      <div class="spinner"></div>
      <div class="hint">首次開啟可能需 30~90 秒（Render 冷啟動）</div>
    </div>
    <script>
      (function () {
        const hideMask = () => {
          const mask = document.getElementById("boot-loader-mask");
          if (!mask) return;
        ...
        const observer = new MutationObserver(() => {
          if (document.querySelector(".stApp")) {
            observer.disconnect();
            setTimeout(hideMask, 240);
          }
        });
        observer.observe(document.documentElement, { childList: true, subtree: true });
        setTimeout(hideMask, 3200);
      })();
    </script>
    """,
